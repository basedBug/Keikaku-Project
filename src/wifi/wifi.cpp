...
{
    /*
        Had to do this fucking mess of functions due to the WIFI library not having
        (or intentionally dropping, the examples and docs do work with the supposed) 
        support of a general event handler

            WiFi.onEvent(WiFiEvent);

        Thus, i had to set every damn handler for each event (with full functions, as
        even the lambdas require providing each event individually)
    */
...
{
    WiFi.onEvent(evWifiReady, ARDUINO_EVENT_WIFI_READY);
    WiFi.onEvent(evWifiStationStarted, ARDUINO_EVENT_WIFI_STA_START);
    WiFi.onEvent(evWifiStationStopped, ARDUINO_EVENT_WIFI_STA_STOP);
    WiFi.onEvent(evWifiStationConnected, ARDUINO_EVENT_WIFI_STA_CONNECTED);
    WiFi.onEvent(evWifiStationDisconnected, ARDUINO_EVENT_WIFI_STA_DISCONNECTED);
    WiFi.onEvent(evWifiStationGotIp, ARDUINO_EVENT_WIFI_STA_GOT_IP);
    WiFi.onEvent(evWifiStationLostIp, ARDUINO_EVENT_WIFI_STA_LOST_IP);
}
